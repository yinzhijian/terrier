...
thread_local std::list<RawBlock *>  local_blocks;
...
DataTable::SlotIterator &DataTable::SlotIterator::operator++() {
  ...
  if (current_slot_.GetOffset() == table_->accessor_.GetBlockLayout().NumSlots() - 1) {
    ...
    current_slot_ = {block_ == local_blocks.end() ? nullptr : *block_, 0};
  } else {
...
DataTable::SlotIterator DataTable::end() const {  // NOLINT for STL name compability
  ...
  if (local_blocks.empty()) return {this, local_blocks.end(), 0};
  auto last_block = --local_blocks.end();
  ...
  if (insert_head == accessor_.GetBlockLayout().NumSlots()) return {this, local_blocks.end(), 0};
